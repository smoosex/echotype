...
自动粘贴说明：启用 `复制后自动粘贴` 后，EchoType 会在转写完成时向当前前台应用发送粘贴操作；在发送粘贴快捷键之前，转写文本也会先写入 macOS 系统剪贴板。
...
说明：当前公开产物默认使用 ad-hoc 签名的可执行文件与资源 bundle，尚未做 Developer ID / notarization。下载或者更新之后，需要执行如下命令，同时需要重新授予麦克风权限和辅助功能权限(辅助功能权限需要删掉后再重新获取)。
...
EchoType 当前使用的模型引擎项目：

- Whisper 系列模型基于 [WhisperKit](https://github.com/argmaxinc/WhisperKit)
- Qwen3-ASR 系列模型基于 [speech-swift](https://github.com/soniqo/speech-swift)
